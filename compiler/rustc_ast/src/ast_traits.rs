...
use crate::{AttrItem, AttrKind, Block, Pat, Path, Restriction, RestrictionKind, Ty, Visibility};
...
impl<Kind: RestrictionKind> HasSpan for Restriction<Kind> {
    fn span(&self) -> Span {
        self.span
    }
}
...
impl<Kind: RestrictionKind> HasAttrs for Restriction<Kind> {
    const SUPPORTS_CUSTOM_INNER_ATTRS: bool = false;
    fn attrs(&self) -> &[Attribute] {
        &[]
    }
