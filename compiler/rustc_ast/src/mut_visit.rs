...
pub trait MutVisitor: Sized {
    ...
    fn visit_restriction<Kind: RestrictionKind>(&mut self, restriction: &mut Restriction<Kind>) {
...
pub fn noop_visit_restriction<T: MutVisitor, Kind: RestrictionKind>(
    restriction: &mut Restriction<Kind>,
    vis: &mut T,
) {
    match &mut restriction.level {
        RestrictionLevel::Unrestricted | RestrictionLevel::Implied => { /* nothing */ }
        RestrictionLevel::Restricted { path, id, shorthand: _ } => {
            vis.visit_path(path);
            vis.visit_id(id);
        }
    }
