...
fn main() {
    ...
    external_top_level.alpha = 1; //~ ERROR field cannot be mutated outside `external_mut_restriction`

    let mut external_inner = external_mut_restriction::inner::Inner::new();
    external_inner.beta = 1; //~ ERROR field cannot be mutated outside `external_mut_restriction`
}
